    ...
    methods: {
        ...
        getCookie(cname) {
          ...
          getList(gameId) {
            ...
            let xml = new XMLHttpRequest()
            xml.open("GET", self.echoParams("https://projektejwkk.com/Covered/API/get/userList.php", {
                userId: userId,
                userAuth: userAuth,
                gameId: gameId
            }))
            ...
            xml.onload = function() {
                let jsonResponse = JSON.parse(this.response)
                self.list = jsonResponse["Message"]
                self.gameName = self.list[0].GameDisplayName
            }
          }
    }, 
    created() {
